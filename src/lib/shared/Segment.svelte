...
<div
  class="flex border bg-primary-400 border-gray-400 dark:border-gray-400 rounded-2xl relative"
>
  {#each options as option}
    <Button
      disabled={option.disabled}
      customStyle={true}
      additionalStyle="flex-1 py-2 text-center transition-colors duration-300 rounded-2xl {selected ===
      option.name
